...
class MoEAlltoAllTokenDispatcher(MoETokenDispatcher):
    ...
    def preprocess(self, routing_map: torch.Tensor) -> torch.Tensor:
        ...
        if self.ep_size > 1 or self.tp_size > 1:
            ...
            num_global_tokens_per_expert = (    # I want to save this to the MoELayer
                gather_from_sequence_parallel_region(
...
class MoEStagedAlltoAllDispatcher(MoEAlltoAllTokenDispatcher):
    def __init__(
        self,
        num_local_experts: int,
        local_expert_indices: List[int],
    ...
    def preprocess(self, routing_map: torch.Tensor) -> torch.Tensor:
        ...
        if self.ep_size > 1 or self.tp_size > 1:
            ...
            # [tp_size, ep_size, num_experts]
            # Stored for access later
            self.num_global_tokens_per_expert = (    # Saved to the MoE layer 
                gather_from_sequence_parallel_region(
                    num_local_tokens_per_expert, group=self.tp_ep_group
                )
                .reshape(self.ep_size, self.tp_size, self.num_experts)
    ...
    def token_dispatch(self, permutated_local_input_tokens, permuted_probs):
        ...
        if (torch.cuda.current_device() == 0):
            print("\n\n\n================= Global matrix\n\n\n")
            print(self.num_global_tokens_per_expert)
            print(self.input_splits)
            print(self.output_splits)
            print("\n\n\n================= Global matrix\n\n\n")
            
        return super().token_dispatch(permutated_local_input_tokens, permuted_probs)
